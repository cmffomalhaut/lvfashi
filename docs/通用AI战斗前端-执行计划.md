...
### 3.23 Tavern 全局脚本浮窗崩溃修复已落地

- 已修复：
  - 全局脚本 iframe 内打开战斗浮窗时，渲染阶段误调用 `getCurrentMessageId()` 导致 `不要对全局脚本 iframe 调用 getMessageId!` 的崩溃
  - MVU 事务层的楼层解析已改为安全回退：楼层界面优先使用 `getCurrentMessageId()`，全局脚本浮窗回退使用 `getLastMessageId()`
  - 战斗浮窗启动失败或运行失败时不再透明消失，而是在浮窗内显示可复制的错误信息
...
- 修复 Tavern 全局脚本浮窗中错误读取楼层 id 导致打开后瞬间空白的问题，并重新打包战斗脚本产物
